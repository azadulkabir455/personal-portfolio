...
export default function ServicesForm() {
  ...
  const confirmRemoveService = (index: number) => {
    confirmDialog.openConfirm({
      ...
      message: "This service will be removed from the list.",
      onConfirm: () => itemsArray.remove(index),
    });
  };
    ...
    >
      ...
      <Container className="flex flex-col gap-4 md:col-span-2">
        ...
        {itemsArray.fields.map((field, index) => (
          <Container key={field.id} className="flex flex-col md:contents">
            <Container className="grid grid-cols-[1fr_auto] gap-4 md:grid-cols-[100px_1fr_auto]">
              <Input
            ...
            {index !== itemsArray.fields.length - 1 && (
              <Container className="h-px bg-[#E4E4E4] md:hidden" />
            )}
          </Container>
        ))}
      </Container>
